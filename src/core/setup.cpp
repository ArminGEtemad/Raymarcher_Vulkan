...
SetupDevice::SetupDevice(WindowHandling &window) : window{window} {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
...
void SetupDevice::createInstance() {
  ...
  if (enableValidationLayers && !checkValidationLayerSupport()) {
    throw std::runtime_error(
        "validation layer is requested, but not available");
  }
...
void SetupDevice::pickPhysicalDevice() {
...
void SetupDevice::createLogicalDevice() {
...
bool SetupDevice::checkValidationLayerSupport() {
  uint32_t layerCount;
...
std::vector<const char *> SetupDevice::getRequiredExtensions() {
...
void SetupDevice::populateDebugMessengerCreateInfo(
...
void SetupDevice::setupDebugMessenger() {
  if (!enableValidationLayers)
    return;
